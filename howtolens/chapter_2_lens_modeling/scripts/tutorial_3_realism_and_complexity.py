...
Therefore, as discussed before, I've included the results of this non-linear search already, allowing you to go 
through the tutorial as if you had actually run them.
...
'autolens_workspace/howtolens/chapter_2_lens_modeling/output/3_realism_and_complexity' folder to the actual values 
...
# %%
correct_fit = al.FitImaging(masked_imaging=masked_imaging, tracer=tracer)

aplt.FitImaging.subplot_fit_imaging(fit=correct_fit)
...
If you did run the phase above yourself, you might of actually inferred the correct lens model. There is some level 
of randomness in a non-linear search. This means that sometimes, it might find a local maxima and other times a 
