# %%
"""
__Non Linear Searches__

Up to now, we've always used the non-lilnear search Dynesty, and we've not considered in detail the different input
parameters that control its sampling. In this tutorial, we'll consider how we can change these setting to balance
finding the global maxima solution with run time, as well as other types of non-linear searches we can use to perform
lens modeling.
"""
...
intially distributed according to our priors, hence why tuning our priors allows us to sample parameter space
faster.
...
n_live_points:
    ...
    The number of live points which sample parameter space. More points provide a more throough sampling of parameter 
...
sampling efficiency:
    ...
    The ratio of accepted to total samples Dynesty targets, for example a sampling_efficiency of 0.6 targets that 60% of
    parameter space samples locate models with a higher log likelihood than the current sample of activate live points. 
    A higher efficiency will converges on the high log_likelihood regions of parameter space faster at the risk of 
    missing the global maxima solution. So, for fast run times, we want to use the highest value possible whilst still
    finding the global maxima solution.
...
evidence_tolerance : float
    ...
    However, a high tolerance can make the errors estimated by Dynesty unreliable - so we should only set the tolerance
    high if we are not interested in errors.
...
Lets perform two fits, where:
    ...
    - One has few live points, a high sampling efficiency and evidence tolerance, causing the non-linear searc to
      convergence and end quicker.
"""
...
phase_slow = al.PhaseImaging(
    ...
    search=af.DynestyStatic(n_live_points=150, sampling_efficiency=0.1, evidence_tolerance=0.8),
)
...
# %%
aplt.FitImaging.subplot_fit_imaging(fit=result_slow.max_log_likelihood_fit, mask=True)
...
phase_fast = al.PhaseImaging(
    ...
    search=af.DynestyStatic(n_live_points=30, sampling_efficiency=0.9, evidence_tolerance=100.0),
)
...
aplt.FitImaging.subplot_fit_imaging(
    fit=result_fast.max_log_likelihood_fit
)
...
# %%
"""
Nested sampling algorithms like Dynesty provides the errors on all of the model parameters, by fully mapping outt all 
of the high likelihood regions of parameter space. This provides knowledge on the complete *range* of models that do 
and do not provide high likelihood fits to the data, but is computationally expense taking more iterations perform. 
If we require precise error estimates (perhaps this is our final lens model fit before we publish the results in a 
paper), this additional computational expense is acceptable. 

However, we often don't care about the errors. For example, in the previous tutorial when linking phases, the only 
result we used from the fit performed in the first phase was the maximum log likelihood model, we ommitted the errors
entirely! Its seems wasteful to use a nested sampling algorithm like Dynesty and map out the entirity of parameter
...
likelihood. They basically want to find the model that maximizes the log likelihood, with no regard for the errors, thus
not wasting the timme to map out in intricate detail every facet of parameter space. Lets see how much faster we can
find a good fit to the lens data using an optimize.

Below, we use the 'Particle Swarm Optimizer' PySwarms. Conceptually this works quite similar to Dynesty, it has a set
of points in parameter space (called 'particles') and it uses their likelihoods to determine where it thinks the higher
...
phase, PySwarms has a tendancy to often only locate a local maxima. Dynesty's slower but more complete sampling of 
parameter space will often find the global maxima when PySwarms doesn't. So, if you're not happy with the results
PySwarms is giving, it may be shrewed to bite-the-button no run-time and use Dynesty to get your initial lens model fit.
...
end with Dynesty for robust errors. Here, choose our non-linear searches based on what result we want and thus how
quickly we can afford to get it!

For users familiar with Markov Chain Monte Carlo (MCMC) non-linear samplers, PyAutoFit supports the non-linear
search *Emcee* (af.Emcee). We have found this to be less effectively or lens modeling than Dynesty and PySwarms,
but it is sill pretty successful. Interested users should feel free to experiment with Emcee!
"""