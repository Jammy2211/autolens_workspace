...
# our phases carefully, we could spend days, or longer, modeling just one grid. In this exercise, we'll get thinking
...
# deflection angle. Convolving a model-with a PSF can take a huge amount of time. As anyone who's written code
...
# The final factor driving run-speed is the shear quantity of datas that we're modeling. For every pixel that we
# fit, we have to compute the light-profile image, the mass-profile deflection angles and convolve it with
# the telescope's PSF. The larger that PSF is, the more convolution operations we have to perform too.

# In the previous exercises, we used with a pixel scale of 0.1". I sneakily chose this value cause its fairly
# low resolution. Most Hubble have a pixel scale of 0.05", which is four times the number of pixels! Some
...
# Of course, there are ways that we can reduce the number of pixels we fit. That's what masking does. If we masks
# out more of the grid, we'll fit fewer pixels and AutoLens will run faster. Alternatively, we could 'bin-up' the
# grid, converting it from say a 0.03" to a 0.06" grid. We lose information, but the code runs
...
# reasonable lens model which fits the datas reasonably well, why bother fitting all the datas?
