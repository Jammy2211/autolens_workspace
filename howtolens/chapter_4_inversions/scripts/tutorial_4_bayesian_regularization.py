...
the image. More specifically, we over-fit the noise in the image, which is what the large flux values located at
 the exteriors of the source reconstruction are doing. Think about it, if your sole aim is to maximize the likelihood, 
...
To determine the best-fit from these solutions the Bayesian evidence quantifies the complexity of each solution's 
...
# %%
"""
Great! As expected, the solution that we could see 'by-eye' was the best solution corresponds to the highest evidence 
solution.
...
However, determining the regularization coefficient that maximizes the Bayesian evidence remains a non-linear problem 
and this becomes part of our non-linear search. The Bayesian evidence also depends on the source resolution which 
