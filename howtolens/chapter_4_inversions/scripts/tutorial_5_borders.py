...
mask_circular = msk.Mask.circular(
...
# border!
mask_annular = msk.Mask.circular_annular(
...
mapper_plotters.plot_image_and_mapper(
    ccd_data=ccd_data, mapper=fit.inversion.mapper, mask=mask_circular, should_plot_grid=True,
...
mask_circular_large = msk.Mask.circular(
...
def simulate_image_x2_lenses():
    ...
    image_plane_grid_stack = grids.GridStack.grid_stack_for_simulation(
        shape=(300, 300), pixel_scale=0.05, psf_shape=(11, 11))
    ...
    source_galaxy_0 = g.Galaxy(
        redshift=1.0,
        light=lp.EllipticalSersic(centre=(0.05, 0.05), axis_ratio=0.8, phi=90.0, intensity=0.2, effective_radius=0.3,
                                  sersic_index=1.0))

    tracer = ray_tracing.TracerImageSourcePlanes(
        lens_galaxies=[lens_galaxy_0, lens_galaxy_1], source_galaxies=[source_galaxy_0],
        image_plane_grid_stack=image_plane_grid_stack)
...
ccd_plotters.plot_ccd_subplot(
    ccd_data=ccd_data, mask=mask_circular, extract_array_from_mask=True, zoom_around_mask=True,
