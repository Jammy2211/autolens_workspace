...
def make_pipeline(phase_folders=None):
    ...
    phase2 = InversionPhase(
        phase_name='phase_2_inversion_initial', phase_folders=phase_folders,
        lens_galaxies=dict(
            lens=gm.GalaxyModel(
                redshift=0.5,
                mass=mp.EllipticalIsothermal)),
    ...
    class InversionPhase(phase_imaging.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens = results.from_phase('phase_2_inversion_initial'). \
                variable.lens_galaxies.lens

            # Note the use of the 'inversion' result attribute below, which uses the extended inversion phase results.

            self.source_galaxies.source = results.from_phase('phase_2_inversion_initial').inversion.\
                constant.source_galaxies.source


    phase3 = InversionPhase(
        phase_name='phase_3_inversion_final', phase_folders=phase_folders,
        lens_galaxies=dict(
            lens=gm.GalaxyModel(
                redshift=0.5,
                mass=mp.EllipticalIsothermal)),
