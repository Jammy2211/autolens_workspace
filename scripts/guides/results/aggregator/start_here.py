...
This script is the starting point for analysing the results of lens modeling using the **aggregator**, which scrapes
a directory of completed fits and loads them on-demand via Python generators. This approach minimises memory usage
and is the right tool when you need to inspect many fits together (e.g. a sample of hundreds of lenses).

If you only want to inspect a single fit quickly, the simpler alternative is to load files directly from the
`output/` folder. See `autolens_workspace/*/guides/results/start_here.py` for that workflow — it loads
