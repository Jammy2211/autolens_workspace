...
For group modeling, there are multiple lens and / or source galaxies. Manually writing the lens model of each in the
Python scripts we used to perform lens model becomes unfeasible and it better for us to manage model composition in
a separate file and store them in .json files.

This script makes the model that is fitted to the example `group` dataset, where:

 - There are three lens galaxies whose light models are `SphSersic` profiles and total mass distributions
...
The group consists of three lens galaxies whose total mass distributions are `SphIsothermal` models.
"""
...
lens_0 = af.Model(al.Galaxy, redshift=0.5, mass=mass)
...
The group has a single source galaxy whose emission is observed but we model as a `PointFlux`.
"""
...
We now combine the the source(s) into a `Collection` object and write it to a `.json` file.
"""
