...
tracer_x2 = al.Tracer(galaxies=[lens_galaxy, lens_galaxy_1, source_galaxy, source_galaxy_1])
...
array_plotter = aplt.Array2DPlotter(
    array=image, visuals_2d=visuals
)
...
multiple_images = solver.solve(tracer=tracer, source_plane_coordinate=source_galaxy.bulge.centre)

visuals = aplt.Visuals2D(multiple_images=multiple_images)

tracer_plotter = aplt.TracerPlotter(
    tracer=tracer, grid=grid, visuals_2d=visuals
)
tracer_plotter.figures_2d(image=True)
...
"""
multiple_images_0 = solver.solve(tracer=tracer, source_plane_coordinate=source_galaxy.bulge.centre)
multiple_images_1 = solver.solve(tracer=tracer, source_plane_coordinate=source_galaxy_1.bulge.centre)
...
light_profile_plotter = aplt.LightProfilePlotter(
    light_profile=tracer.galaxies[0].bulge, grid=grid, mat_plot_1d=mat_plot, visuals_1d=visuals
)
light_profile_plotter.figures_1d(image=True)
...
"""
visuals = aplt.Visuals1D(einstein_radius=tracer.galaxies[0].mass.einstein_radius_from(grid=grid))
...
mass_profile_plotter = aplt.MassProfilePlotter(
    mass_profile=tracer.galaxies[0].mass, grid=grid, mat_plot_1d=mat_plot, visuals_1d=visuals
)
mass_profile_plotter.figures_1d(convergence=True)
