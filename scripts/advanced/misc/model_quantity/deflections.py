...
The reasons for doing such a fit are because one can fit a property of a certain mass profile (e.g. the convergence
of an `NFW` mass profile) to the same property of a different mass profile (e.g. the convergence of a `PowerLaw`).
This allows one to infer parameters describing how to translate between two mass profiles as closely as possible, and
to understand how similar or different the mass profiles are.
...
We assume a noise-map where all values are arbitrarily 0.01.
"""
...
dataset = al.DatasetQuantity(
    ...
    noise_map=al.Array2D.full(
        fill_value=0.01,
        shape_native=deflections_2d.shape_native,
        pixel_scales=deflections_2d.pixel_scales,
    ),
)
...
The model-fit requires a 2D mask defining the regions of the deflections we fit, which we define and apply to the 
...
search = af.Nautilus(
    ...
    name="quantity_via_deflections_fit",
    n_live=100,
)
