...
@numba.jit(nopython=True, cache=False, parallel=True)
def real_visibilities_from_image_via_sums(image_1d, preloaded_reals):

    real_visibilities = np.zeros(shape=(preloaded_reals.shape[1]))

    for image_1d_index in range(image_1d.shape[0]):
        real_visibilities[:] += (
            image_1d[image_1d_index] * preloaded_reals[image_1d_index, :]
        )
...
for total_visibilities in [100, 1000, 5000, 10000, 25000]:
    ...
    if total_visibilities == 100:
        vis1 = real_visibilities_from_image_via_sums(
            image_1d=profile_image, preloaded_reals=preloaded_real_transforms
        )
        vis2 = al.util.transformer.real_visibilities_from_image_via_preload_jit(
            image_1d=profile_image, preloaded_reals=preloaded_real_transforms
        )
        assert vis1 == pytest.approx(vis2, 1.0e-4)

    start = time.time()
    for i in range(repeats):
        real_visibilities_from_image_via_sums(
            image_1d=profile_image, preloaded_reals=preloaded_real_transforms
        )
    diff = time.time() - start
