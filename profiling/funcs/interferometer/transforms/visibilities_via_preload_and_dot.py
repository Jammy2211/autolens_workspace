...
def real_visibilities_from_image_via_dot(image_1d, preloaded_reals):

    real_visibilities = np.zeros(shape=(preloaded_reals.shape[1]))

    for image_1d_index in range(image_1d.shape[0]):
        real_visibilities += np.dot(
            a=image_1d[image_1d_index], b=preloaded_reals[image_1d_index, :]
        )
...
for total_visibilities in [
    ...
    start = time.time()
    for i in range(repeats):
        real_visibilities_from_image_via_dot(
