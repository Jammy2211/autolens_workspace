...
data_type = 'example'
data_name = 'lens_bulge_disk_mass_and_x1_source' # Example simulated image with lens light emission and a source galaxy.
pixel_scale = 0.1
...
from workspace.pipelines.hyper.with_lens_light.bulge_disk.initialize import lens_sersic_exp_sie_shear_source_sersic
from workspace.pipelines.hyper.with_lens_light.bulge_disk.inversion.from_initialize import lens_sersic_exp_sie_shear_source_inversion
from workspace.pipelines.hyper.with_lens_light.bulge_disk.power_law.from_inversion import lens_sersic_exp_pl_shear_source_inversion

pipeline_initialize = lens_sersic_exp_sie_shear_source_sersic.make_pipeline(
    pl_hyper_galaxies=True, pl_hyper_background_sky=False, pl_hyper_background_noise=False,
    pl_align_bulge_disk_centre=True, pl_align_bulge_disk_axis_ratio=True,
    pl_align_bulge_disk_phi=True, phase_folders=[data_type, data_name])

pipeline_inversion = lens_sersic_exp_sie_shear_source_inversion.make_pipeline(
    pl_hyper_galaxies=True, pl_hyper_background_sky=False, pl_hyper_background_noise=False,
    pl_fix_lens_light=True, pl_align_bulge_disk_centre=True,
