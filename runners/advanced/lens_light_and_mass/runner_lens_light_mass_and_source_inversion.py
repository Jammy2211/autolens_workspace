...
# - If an ExternalShear is fitted for throughout the pipeline (Defaut True).
...
pipeline_settings = pl.PipelineSettings(
    include_shear=True,
...
from workspace.pipelines.advanced.with_lens_light.sersic.inversion.from_initialize import (
    lens_sersic_sie_source_inversion,
)
from workspace.pipelines.advanced.with_lens_light.sersic.power_law.from_inversion import (
    lens_sersic_pl_source_inversion,
)

pipeline_initialize = lens_sersic_sie_source_sersic.make_pipeline(
    pipeline_settings=pipeline_settings, phase_folders=[data_type, data_name]
)

pipeline_inversion = lens_sersic_sie_source_inversion.make_pipeline(
    pipeline_settings=pipeline_settings, phase_folders=[data_type, data_name]
)

pipeline_power_law = lens_sersic_pl_source_inversion.make_pipeline(
    pipeline_settings=pipeline_settings, phase_folders=[data_type, data_name]
)
