...
In this pipeline, we fit `Imaging` of a strong lens system where:
 ...
 - The lens `Galaxy`'s stellar `MassProfile` is fitted with the `EllipticalSersic` and `EllipticalExponential` of the 
      `LightProfile`, where they are converted to a stellar mass distribution via constant mass-to-light ratios.
...
def make_pipeline(setup, settings):
    ...
    """Setup: Align the centre of the bulge `LightProfile` and dark matter `MassProfile` if input in _SetupMassLightDark_."""
