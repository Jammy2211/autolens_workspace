...
def make_pipeline(phase_folders=None, tag_phases=True, sub_grid_size=2, bin_up_factor=None, positions_threshold=None,
    ...
    pipeline_name = 'pipeline_init__lens_sersic_exp_sie_shear_source_sersic' + bulge_disk_tag
    ...
    class BulgeDiskPhase(ph.LensPlanePhase):

        def pass_priors(self, results):

            if align_bulge_disk_centre:
                self.lens_galaxies.lens.bulge.centre = self.lens_galaxies.lens.disk.centre

            if align_bulge_disk_axis_ratio:
                self.lens_galaxies.lens.bulge.axis_ratio = self.lens_galaxies.lens.disk.axis_ratio

            if align_bulge_disk_phi:
                self.lens_galaxies.lens.bulge.phi = self.lens_galaxies.lens.disk.phi

    phase1 = BulgeDiskPhase(phase_name='phase_1_lens_sersic_exp',
    ...
    class LensSubtractedPhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.bulge = results.from_phase('phase_1_lens_sersic_exp').constant.lens.bulge
            self.lens_galaxies.lens.disk = results.from_phase('phase_1_lens_sersic_exp').constant.lens.disk

            ## Lens Mass, Move centre priors to centre of lens light ###

            self.lens_galaxies.lens.mass.centre = \
                results.from_phase("phase_1_lens_sersic_exp").variable_absolute(a=0.1).lens.bulge.centre

    phase2 = LensSubtractedPhase(phase_name='phase_2_lens_sie_shear_source_sersic',
                                 phase_folders=phase_folders, tag_phases=tag_phases,
                                 lens_galaxies=dict(lens=gm.GalaxyModel(bulge=lp.EllipticalSersic,
                                                                        disk=lp.EllipticalExponential,
    ...
    class LensSourcePhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.bulge = results.from_phase("phase_1_lens_sersic_exp").variable.lens.bulge
            self.lens_galaxies.lens.disk = results.from_phase("phase_1_lens_sersic_exp").variable.lens.disk
    ...
    phase3 = LensSourcePhase(phase_name='phase_3_lens_sersic_exp_sie_shear_source_sersic',
                             phase_folders=phase_folders, tag_phases=tag_phases,
                             lens_galaxies=dict(lens=gm.GalaxyModel(bulge=lp.EllipticalSersic,
                                                                    disk=lp.EllipticalExponential,
                                                                    mass=mp.EllipticalIsothermal,
                             ...
                             optimizer_class=nl.MultiNest,
                             sub_grid_size=sub_grid_size, bin_up_factor=bin_up_factor,
                             positions_threshold=positions_threshold, interp_pixel_scale=interp_pixel_scale)
