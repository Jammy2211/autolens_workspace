...
def make_pipeline(
    ...
    class LensSourcePhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.mass.einstein_radius_in_units = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
                variable_absolute(a=0.3).lens_galaxies.lens.mass.einstein_radius_in_units

            ### Lens Shear, Shear -> Shear ###

            self.lens_galaxies.lens.mass.shear = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
                variable.lens_galaxies.lens.shear

            ### Source Light, Sersic -> Sersic ###

            self.source_galaxies.source.light.centre = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
                variable_absolute(a=0.3).source_galaxies.source.light.centre

            self.source_galaxies.source.light.intensity = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
                variable.source_galaxies.source.light.intensity

            self.source_galaxies.source.light.effective_radius = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
                variable_absolute(a=1.0).source_galaxies.source.light.effective_radius

            self.source_galaxies.source.light.sersic_index = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
                variable_absolute(a=1.5).source_galaxies.source.light.sersic_index

            self.source_galaxies.source.light.axis_ratio = results.from_phase('phase_3_lens_sersic_sie_shear_source_sersic').\
