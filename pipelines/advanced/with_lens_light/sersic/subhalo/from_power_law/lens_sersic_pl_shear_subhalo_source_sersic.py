...
def make_pipeline(
    ...
    class GridPhase(af.phase.as_grid_search(phase_imaging.LensSourcePlanePhase)):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.light = results.from_phase(
                "phase_1_lens_sersic_pl_shear_source_sersic"
            ...
            self.source_galaxies.source.light.centre = (
                results.from_phase("phase_1_lens_sersic_pl_shear_source_sersic")
                .variable_absolute(a=0.3)
                .source_galaxies.source.light.centre
            )

            self.source_galaxies.source.light.intensity = results.from_phase(
                "phase_1_lens_sersic_pl_shear_source_sersic"
            ).variable.source_galaxies.source.light.intensity

            self.source_galaxies.source.light.effective_radius = results.from_phase(
                "phase_1_lens_sersic_pl_shear_source_sersic"
            ).variable.source_galaxies.source.light.effective_radius

            self.source_galaxies.source.light.sersic_index = results.from_phase(
                "phase_1_lens_sersic_pl_shear_source_sersic"
            ).variable.source_galaxies.source.light.sersic_index

            self.source_galaxies.source.light.axis_ratio = results.from_phase(
                "phase_1_lens_sersic_pl_shear_source_sersic"
            ).variable.source_galaxies.source.light.axis_ratio

            self.source_galaxies.source.light.phi = results.from_phase(
                "phase_1_lens_sersic_pl_shear_source_sersic"
    ...
    phase2 = SubhaloPhase(
        phase_name="phase_3_subhalo_refine",
        phase_folders=phase_folders,
        tag_phases=tag_phases,
        lens_galaxies=dict(
            lens=gm.GalaxyModel(
