...
# ''pipelines/no_lens_light/inversion/lens_sie_shear_source_inversion_from_initialize.py'.
...
# Previous Pipelines: no_lens_light/inversion/lens_sie_shear_source_inversion_from_initialize.py
...
def make_pipeline(
    ...
    class LensSourcePhase(phase_imaging.LensSourcePlanePhase):
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.mass.centre = results.from_phase(
                "phase_4_lens_sie_shear_source_inversion"
            ).variable.lens_galaxies.lens.mass.centre

            self.lens_galaxies.lens.mass.axis_ratio = results.from_phase(
                "phase_4_lens_sie_shear_source_inversion"
            ).variable.lens_galaxies.lens.mass.axis_ratio

            self.lens_galaxies.lens.mass.phi = results.from_phase(
                "phase_4_lens_sie_shear_source_inversion"
            ).variable.lens_galaxies.lens.mass.phi

            self.lens_galaxies.lens.mass.einstein_radius = (
                results.from_phase("phase_4_lens_sie_shear_source_inversion")
                .variable_absolute(a=0.3)
                .lens_galaxies.lens.mass.einstein_radius
            )

            ### Lens Shear, Shear -> Shear ###

            self.lens_galaxies.lens.phase_2_lens_sie_shear_source_magnification_inversion = results.from_phase(
                "phase_4_lens_sie_shear_source_inversion"
    ...
    phase1 = LensSourcePhase(
        phase_name="phase_1_lens_pl_shear_source_inversion",
