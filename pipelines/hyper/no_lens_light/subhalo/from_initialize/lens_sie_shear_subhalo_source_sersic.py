...
def make_pipeline(
    ...
    class GridPhase(af.phase.as_grid_search(phase_imaging.LensSourcePlanePhase, parallel=parallel)):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.subhalo.mass.kappa_s = af.prior.UniformPrior(lower_limit=0.0, upper_limit=1.0)
            ...
            self.source_galaxies.source.light.centre = results.from_phase('phase_1_lens_sie_shear_source_sersic').\
                variable_absolute(a=0.1).source_galaxies.source.light.centre

            self.source_galaxies.source.light.intensity = results.from_phase('phase_1_lens_sie_shear_source_sersic').\
                variable_relative(r=0.5).source_galaxies.source.light.intensity

            self.source_galaxies.source.light.effective_radius = results.from_phase('phase_1_lens_sie_shear_source_sersic').\
                variable_relative(r=0.5).source_galaxies.source.light.effective_radius

            self.source_galaxies.source.light.sersic_index = results.from_phase('phase_1_lens_sie_shear_source_sersic').\
                variable_relative(r=0.5).source_galaxies.source.light.sersic_index

            self.source_galaxies.source.light.axis_ratio = results.from_phase('phase_1_lens_sie_shear_source_sersic').\
                variable_absolute(a=0.1).source_galaxies.source.light.axis_ratio

            self.source_galaxies.source.light.phi = results.from_phase('phase_1_lens_sie_shear_source_sersic').\
    ...
    phase1 = GridPhase(
        ...
        optimizer_class=af.MultiNest, number_of_steps=5)
    ...
    class SubhaloPhase(phase_imaging.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.source_galaxies.source.light.centre = results.from_phase('phase_1_subhalo_search').\
                best_result.variable_absolute(a=0.05).source_galaxies.source.light.centre

            self.source_galaxies.source.light.intensity = results.from_phase('phase_1_subhalo_search').\
                best_result.variable_relative(r=0.5).source_galaxies.source.light.intensity

            self.source_galaxies.source.light.effective_radius = results.from_phase('phase_1_subhalo_search').\
                best_result.variable_relative(r=0.5).source_galaxies.source.light.effective_radius

            self.source_galaxies.source.light.sersic_index = results.from_phase('phase_1_subhalo_search').\
