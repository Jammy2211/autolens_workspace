...
# ''pipelines/with_lens_light/inversion/from_initialize/lens_sersic_sie_source_inversion.py'.
...
# Previous Pipelines: with_lens_light/inversion/from_initialize/lens_sersic_sie_source_inversion.py
...
def make_pipeline(
    ...
    pipeline_name = "pipeline_pl__lens_sersic_pl_source_inversion"
    ...
    class LensSourcePhase(phase_imaging.PhaseImaging):
        def pass_priors(self, results):
            ...
            self.galaxies.lens.light = results.from_phase(
                "phase_4_lens_sersic_sie_source_inversion"
            ).variable.galaxies.light

            ### Lens Mass, SIE -> Powerlaw ###

            self.galaxies.lens.mass.centre = (
                results.from_phase("phase_4_lens_sersic_sie_source_inversion")
    ...
    phase1 = LensSourcePhase(
        phase_name="phase_1_lens_sersic_pl_source_inversion",
