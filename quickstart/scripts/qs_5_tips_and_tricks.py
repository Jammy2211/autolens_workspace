...
Its time to model your own lens data! This is straight forward - just adapt the runner script you ran above to load
your own imaging data instead of the example data we ran previously.
...
# %%
"""
2) Creating your own custom mask, using the script 'autolens_workspace/preprocessing/gui/mask_maker.py'


If your data doesn't contain the lens galaxy's light (this is often the case for radio / sub-mm imaging of strong 
lenses, where only the source galaxy is visible) then you should use the 'runner_lens_mass_and_source.py' script 
instead.
"""
...
one another (which is controlled by the 'position_threshold' input into a phase). This provides two benefits:
...
For setting up an image with positions, checkout 'autolens_workspace/preprocessing/gui/positions_maker.py'. To see how 
positions are used by a runner and pipeline, checkout 'pipelines/features/position_thresholding.py' and 
...
So far, all the models we've run have used analytic light profiles to fit for the source galaxy (e.g. Sersic's). 
However, most strongly lensed sources are more complex then these simplistic symmetric light profiles, and our 
lens modeling benefits from reconstructing the source using a more general approach, called an 'inversion'. This 
basically reconstructs the source's light using a pixelized-grid.

To use an inversion, checkout the pipeline:
 
'pipelines/beginner/with_lens_light/lens_sersic_sie__source_inversion.py' 

and the runner: 

'runners/beginner/with_lens_light/lens_sersic_sie__source_inversion.py'. 

By default, these pipelines use an adaptive pixelization whereby the inversion's pixels adapt to the surface 
brightness of the source.

I would strongly recommend you only use inversions in conjuction with the positions feature described above. Inversions 
run a high risk of going to incorrect solutions if this feature is not used! 
...
1) 'pipelines/features' and 'runners/features'. These describe pipeline features that customize an analysis of a 
    strong lens, for example by binning up the data to a coarser resolution of performing ray-tracing on a higher 
    resolution 'sub-grid'.

2) 'pipelines/advanced' and 'runners/advanced'. These advanced pipelines and runners allow for a much broader range of 
    complex lens models to be fitted, and allow for different pipelines to be combined together so that the early 
...
lens model you want, you should checkout the howtolens lecture series for a complete description of how to use 
PyAutoLens.
"""
