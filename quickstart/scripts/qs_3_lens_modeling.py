...
# of parameters provided a good fit to the data, another set with similar values probably will too!
...
# First, we load the CCD data we're going to it, so you'll again need to change the path below to that of
# your workspace.
...
# take 10 minutes or so. Whilst you wait, lets explore the workspace:
...
# 3) Look at 'autolens_workspace/data'. This is where the data we load and fit is stored (e.g. .fits files of the
#    image, noise-map and PSF).

# 4) Finally, checkout 'autolens_workspace/tools'. This contains Python scripts for standard lensing tasks, such as
...
# From here, you can use template pipelines to fit your own strong lens data. The howtolens lecture series has
