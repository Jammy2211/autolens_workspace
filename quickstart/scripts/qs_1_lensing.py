...
image_plane_grid = grids.RegularGrid.from_shape_and_pixel_scale(
    shape=(100, 100), pixel_scale=0.05)

grid_plotters.plot_grid(
    grid=image_plane_grid, title='Image-Plane Uniform Grid')
...
sis_mass_profile = mp.EllipticalIsothermal(
    centre=(0., 0.), axis_ratio=0.9, phi=45.0, einstein_radius=1.6)
...
grid_plotters.plot_grid(
    grid=source_plane_grid, title='Source Plane Lensed Grid')
...
profile_plotters.plot_intensities(
    light_profile=sersic_light_profile, grid=image_plane_grid,
    title='Image-Plane Sersic Profile Image')

profile_plotters.plot_intensities(
    light_profile=sersic_light_profile, grid=source_plane_grid,
...
galaxy_plotters.plot_intensities(
    galaxy=galaxy, grid=image_plane_grid)
...
source_galaxy = g.Galaxy(
    redshift=1.0,
    light=lp.SphericalSersic(centre=(0.0, 0.0), intensity=1.0, effective_radius=1.0, sersic_index=1.0))
