...
# in the file 'pipelines/simple/lens_sersic_sie__source_sersic.py'. The results of this fit are distributed in
...
# To begin, we setup the path to the autolens_workspace and our output folder.
...
sersic_indexes = list(
    map(
        lambda most_likely: most_likely.galaxies.lens.light.sersic_index,
...
einstein_radii = list(
    map(
        lambda most_probable: most_probable.galaxies.lens.mass.einstein_radius,
...
einstein_radii_upper_errors = list(
    map(lambda upper: upper.galaxies.lens.mass.einstein_radius, upper_error_instances)
)
einstein_radii_lower_errors = list(
    map(lambda lower: lower.galaxies.lens.mass.einstein_radius, lower_error_instances)
)
...
luminosities = list(
    map(
        lambda most_probable: most_probable.galaxies.lens.light.luminosity_within_circle_in_units(
            radius=1.0
        ),
        most_probable_model_instances,
    )
...
masses = list(
    map(
        lambda most_probable: most_probable.galaxies.lens.mass_within_circle_in_units(
            radius=1.0
        ),
        most_probable_model_instances,
    )
